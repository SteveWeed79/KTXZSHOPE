...
import { auth, signOut } from "@/auth"; // Corrected alias
    ...
    <nav className="border-b border-gray-900 p-4 sticky top-0 z-50 bg-black/80 backdrop-blur-md">
      <div className="max-w-7xl mx-auto flex justify-between items-center gap-8">
        ...
        <div className="flex items-center gap-4">
          <Link href="/" className="text-2xl font-black tracking-tighter hover:text-red-600 transition-colors italic uppercase">
            KTXZ
          </Link>

          {(session?.user?.email === "steveweed1979@gmail.com" || (session?.user as any)?.role === "admin") && (
            <Link href="/admin" className="text-[9px] text-red-500 font-bold border border-red-500/30 px-2 py-1 rounded hover:bg-red-600 hover:text-white transition-all tracking-tighter uppercase">
              System Admin
            </Link>
          )}
        </div>
