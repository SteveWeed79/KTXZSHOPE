...
export async function PUT(
  ...
  try {
    ...
    let body: { role?: unknown; confirmPassword?: unknown };
    try {
      body = await req.json();
    } catch {
    ...
    const { role, confirmPassword } = body;

    // Step-up auth required for role changes
    const stepUpError = await requireStepUpAuth(adminResult, confirmPassword as string | undefined);
